...
export default function main({ BtApp, BtPage }) {
  test('[BtPlugin] options.relyOn', t => {
    ...
    t.is(
      error.message,
      `[BeautyWe:error] 有 1 个 ${pluginA.name} 的依赖插件未预注册`
    );
...
//         {
//           name: 'pluginC',
//           npmName: '@beautywe/plugin-pluginC',
//           version: '0.0.2',
//         },
//       ],
//     };
...
//     const pluginC_version001 = {
//       name: 'pluginC',
//       npmName: '@beautywe/plugin-pluginC',
//       version: '0.0.1',
//     };
//     // eslint-disable-next-line
//     const pluginC_version002 = {
//       name: 'pluginC',
//       npmName: '@beautywe/plugin-pluginC',
//       version: '0.0.2',
//     };
...
//         t.is(
//           error.message,
//           `[BeautyWe:error] 有 2 个 ${pluginA.name} 的依赖插件未预注册`
//         );
//       })
//       .then(() => {
//         const app = new BtApp();
...
//         msg: msg3,
//         context: context3,
//       } = app.pluginA.level2.level3.sayHello();
//       t.is(msg, 'hello');
//       t.is(msg2, 'hello');
//       t.is(msg3, 'hello');
//       t.is(context, app);
...
//         t.is(
//           error.message,
//           `[BeautyWe:error] theHost.data.${pluginA.name} 命名空间被占用了，插件 ${pluginA.name} 需要用到该命名空间，你可以更改插件名（plugin.name = xxx）或者手动更改宿主 data 部分来处理冲突`
//         );
//       })
//       .then(() => {
...
//         t.is(
//           error.message,
//           `[BeautyWe:error] theHost.${pluginA.name} 命名空间被占用了，插件 ${pluginA.name} 需要用到该命名空间，你可以更改插件名（plugin.name = xxx）或者手动处理冲突`
//         );
//       });
//   });
...
//     t.is(
//       error.message,
//       '[BeautyWe:error] params of name are required when create a BtPlugin.'
//     );
//     t.end();
//   });
...
//       .then(() => page.onLoad())
//       .then(results => {
//         t.deepEqual(results, ['page', 'pluginA', 'pluginB']);
//         t.truthy(status.customMethodBinded);
//         t.truthy(
//           status.timestamp.onLoad.page < status.timestamp.onLoad.pluginA
//         );
//         t.truthy(
//           status.timestamp.onLoad.pluginA < status.timestamp.onLoad.pluginB
//         );
//       });
//   });
...
//     return Promise.resolve()
//       .then(() => Promise.all([app.onLaunch(), app.onShow()]))
//       .then(() => {
...
//     t.is(
//       error.message,
//       `[BeautyWe:error] ${pluginA.name} 已经注册了，不允许重复注册`
//     );
//   });
