...
function getNotIncludedPlugins({ theHost, relyOn }) {
  ...
  function wasIncluded(relyConf, plugin) {
    ...
    const npmNameEqual = relyConf.npmName
      ? relyConf.npmName === plugin.npmName
      : true;
    const versionEqual = relyConf.version
      ? relyConf.version === plugin.version
      : true;
...
function attachPlugin({ theHost, plugin }) {
  ...
  if (theHost.data[plugin.name])
    throw new BtError(
      `theHost.data.${plugin.name} 命名空间被占用了，插件 ${plugin.name} 需要用到该命名空间，你可以更改插件名（plugin.name = xxx）或者手动更改宿主 data 部分来处理冲突`
    );
  if (theHost[plugin.name])
    throw new BtError(
      `theHost.${plugin.name} 命名空间被占用了，插件 ${plugin.name} 需要用到该命名空间，你可以更改插件名（plugin.name = xxx）或者手动处理冲突`
    );
  ...
  if (plugin.content.nativeHook) {
    ...
    Object.keys(nativeHooks).forEach(funName => {
      if (typeof nativeHooks[funName] !== 'function') {
        logger.warn(
          `nativeHook attach fail，the native hook(${funName}) should be a function`
        );
        return;
      }
      ...
      if (!theHost.getHookFunQueue(funName)) {
        logger.warn(
          `nativeHook attach fail，the native hook(${funName}) not be support on the host`
        );
        return;
      }
  ...
  if (plugin.content.handler) {
    ...
    Object.keys(handlerHooks).forEach(funName => {
      ...
      if (!theHost.getHookFunQueue(funName)) {
        let originFun;
        if (typeof theHost[funName] === 'function')
          originFun = theHost[funName];
...
class Host {
  ...
  use(plugins) {
    ...
    for (let index = 0; index < plgs.length; index += 1) {
      ...
      if (
        theHost._btPlugin.plugins.map(item => item.name).includes(plugin.name)
      ) {
      ...
      if (relyOn && Array.isArray(relyOn)) {
        const thePluginsWasNotIncluded = getNotIncludedPlugins({
          theHost,
          relyOn,
        });
        if (thePluginsWasNotIncluded.length > 0) {
          thePluginsWasNotIncluded.forEach(need => {
            ...
            if (need.npmName) {
              const installMessage = need.version
                ? `${need.npmName}@${need.version}`
                : `${need.npmName}`;
          ...
          throw new BtError(
            `有 ${thePluginsWasNotIncluded.length} 个 ${plugin.name} 的依赖插件未预注册`
          );
        }
      }
